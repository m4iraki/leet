  ...
  def threeSum(nums: Array[Int]): List[List[Int]] = {
    ...
    def loop(
      ...
      if idx >= len - 2 then acc
        ...
        val fst = sorted(idx)
        if l >= r then {
          val next = search(idx, 1, len - 2, fst)
          ...
          val sum = fst + snd + trd
          if sum == 0 then {
            val nextL = search(l, 1, r, snd)
            val nextR = search(r, -1, l, trd)
            loop(idx, nextL, nextR, List(fst, snd, trd) :: acc)
          } else if sum > 0 then loop(idx, l, search(r, -1, l, trd), acc)
          else loop(idx, search(l, 1, r, snd), r, acc)
        }
      }
